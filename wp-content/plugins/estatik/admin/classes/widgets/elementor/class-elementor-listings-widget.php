...
class Elementor_Es_Listings_Widget extends Elementor_Es_Base_Widget {
    ...
    protected function _register_controls() {
        ...
        foreach ( array( 'country', 'state', 'province', 'city', 'neighborhood' ) as $location ) {
            ...
            if ( ! empty( $finfo ) ) {
                $location_type = $finfo['formatter'];
                $locations = ES_Address_Components::get_component_list( $location_type );
                $locations = $locations ? wp_list_pluck( $locations, 'long_name', 'id' ) : array();

                $this->add_control( $location, array(
                    'label' => __( ES_Address_Components::get_label_by_type( $location_type ), 'es-plugin' ),
                    'type' => Controls_Manager::SELECT2,
                    'multiple' => 'multiple',
                    'options' => $locations,
                ) );
